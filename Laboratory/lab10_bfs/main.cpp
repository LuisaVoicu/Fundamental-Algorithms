        ...
        for(j=0; j<grid->cols; ++j){
            if (rainbow == 1)
            {
                if (grid->mat[i][j] == -1)
                {
                    ...
                    if (grid->mat[i][j] > 0)
                    {
                        set_text_color(0, grid->mat[i][j] % 15 + 1);
                        char* s = (char*)malloc(sizeof(char) * 2);
                        s[0] = (grid->mat[i][j] % 100) / 10 + '0';
                        s[1] = grid->mat[i][j] % 10 + '0';
                        s[2] = '\0';

                        printf(s);
                    }
                    else if (grid->mat[i][j] == 0)
                    {
                        set_text_color(0, 0);
                        printf("  ");
                    }
    ...
    do{
        ...
        if(strcmp(token, "exit") == 0 || strcmp(token, "quit") == 0 || strcmp(token, "q") == 0){
            ...
            validCommand = 1;
                ...
                strcmp(token, "path") == 0 || strcmp(token, "knight") == 0 || strcmp(token, "knight_free") == 0){
            if(strcmp(token, "neighb") == 0 || strcmp(token, "neighbors") == 0){
                ...
                c->command = COMMAND_KNIGHT;
            }else if(strcmp(token,"knight_free") == 0){
                c->command = COMMAND_KNIGHT_FREE_SPACE;
            }
...
void knight_moves(Grid* grid, Node* s)
{
    ...
    grid->mat[s->position.row][s->position.col] = 1;
    while (Q.head)
    {
        Node* u = dequeue(&Q);

        if (grid->mat[u->position.row][u->position.col] != -1 )
        {
            ...
            for (int k = 0; k < 8; k++)
            {
                ...
                {
                    printf("\n\n");
                    grid->mat[newPos.row][newPos.col] = grid->mat[u->position.row][u->position.col] + 1;
                    Node* node = make_node_from_position(newPos);
                    enqueue(&Q, node);
                    displayGrid(grid, cmd.command);
                    sleep(0.0001);
                }
...
int main()
{
    ...
    for(;;){
        ...
        if (cmd.command == COMMAND_KNIGHT || cmd.command == COMMAND_KNIGHT_FREE_SPACE)
        {
            rainbow = 1;
        }
        ...
        if(cmd.command == COMMAND_EXIT){
            ...
            performance();
        }else if(cmd.command == COMMAND_KNIGHT){
            for (int indexRow = 0; indexRow < grid.rows; indexRow++)
            {
                for (int indexCol = 0; indexCol < grid.cols; indexCol++)
                {
                    grid2.mat[indexRow][indexCol] = 0;
                }
            }

            Point p = { cmd.p1.row,cmd.p1.col };
            Node* nodePos0 = make_node_from_position(p);
       
            knight_moves(&grid2, nodePos0);
        }else if(cmd.command == COMMAND_KNIGHT_FREE_SPACE){
            for (int indexRow = 0; indexRow < grid.rows; indexRow++)
            {
                for (int indexCol = 0; indexCol < grid.cols; indexCol++)
                {
            ...
            for (int k = 0; k < 8; k++)
            {
                int nx = crtX + positionX[k];
                int ny = crtY + positionY[k];
                grid2.mat[nx][ny] = -1;
            }
